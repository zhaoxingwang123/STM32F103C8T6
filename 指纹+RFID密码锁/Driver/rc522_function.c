 ...
 * 描述  ：向RC522发送1 Byte 数据在SCK的上升沿，主设备将数据位放在MOSI线上
 * 输入  ：byte，要发送的数据
 * 返回  : RC522返回的数据
 * 调用  ：内部调用
 */
...
u8 SPI_RC522_ReadByte( void )
{
    ...
    for(counter=0;counter<8;counter++){
        ...
        macRC522_SCK_0 ();      //低电平期间读取
        macRC522_DELAY();
        
        if ( macRC522_MISO_GET() == 1)
            SPI_Data |= 0x01;   //保存在SPI_Data

        macRC522_DELAY();
        macRC522_SCK_1();      //将其置为高
        macRC522_DELAY();
    }
    return SPI_Data;
}
...
u8 ReadRawRC ( u8 ucAddress ){
    ...
    ucAddr = ( ( ucAddress << 1 ) & 0x7E ) | 0x80;      //最高位为1表示读
...
{  
    ...
    ucAddr = ( ucAddress << 1 ) & 0x7E;                 //最高位为1表示写
...
void PcdReset ( void )
{
    ...
    while ( ReadRawRC(CommandReg) & 0x10 );     //等待准备好
